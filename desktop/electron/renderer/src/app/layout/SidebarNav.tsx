...
const navItems: Array<{ to: string; label: string; icon: string }> = [
  ...
  { to: '/console', label: '控制台', icon: 'console' },
  { to: '/reports', label: '报告', icon: 'reports' },
  { to: '/cluster', label: '聚类', icon: 'cluster' },
  { to: '/kb', label: '知识库', icon: 'kb' },
...
export function SidebarNav() {
  return (
    <aside className="nav">
      ...
      <nav className="nav-items" role="navigation" aria-label="主导航">
        {navItems.map((item) => (
          <NavLink
            key={item.to}
            to={item.to}
      ...
      <div style={{ marginTop: 'auto', paddingTop: 16, borderTop: '1px solid var(--border-default)' }}>
        <div className="muted" style={{ fontSize: 10, textAlign: 'center', letterSpacing: '0.3px' }}>
          v1.0.0 · Nightly
        </div>
      </div>
    </aside>
