...
export function getClustersRoot(userDataPath: string): string {
  return path.join(userDataPath, 'results', 'clusters')
}

export function createClusterStore(args: { userDataPath: string; fs: ClusterStoreFs; now?: () => number }) {
  const now = args.now ?? (() => Date.now())
  const root = getClustersRoot(args.userDataPath)
  ...
  const deleteCluster = (input: { dirName: string; names?: string[] }) => {
    const dirPath = resolveDir(input.dirName)
  ...
  return { root, list, save, readFile, delete: deleteCluster, copyToDir }
}
