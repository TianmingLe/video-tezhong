        ...
        async def _run_one(i: int, c: VideoCandidate) -> None:
            ...
            async with semaphore:
                ...
                while True:
                    attempts += 1
                    try:
                        if not self.single_runner:
                            raise RuntimeError("single_runner is required for non-dry-run execution")
                        await self.single_runner.run_one(index=i, candidate=c, run_context=self.run_context)
                        ...
                        if attempts <= max_retries:
                            ...
                            continue
                        registry.append_failed(aweme_id=c.aweme_id, failed_stage="unknown", error_code=str(e))
                        self._emit(
                            ProgressEvent(
                                index=i,
                                total=len(ordered),
